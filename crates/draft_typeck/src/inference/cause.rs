...
use draft_common::{span::Span, bail};
    ...
    pub(crate) fn into_typeck_error(
        ...
        match error {
            CoercionError::RecursiveType => match self {
                Cause::StructArgument(span)
                | Cause::VariantArgument(span)
                | Cause::FunctionArgument(span)
                | Cause::Assignment(span)
                | Cause::Return(span, _)
                | Cause::Pattern(span, _)
                | Cause::Branch(span, _) => bail!(TypeckError::RecursiveType {
            ...
            CoercionError::TypesMissmatch => match self {
                ...
                | Cause::VariantArgument(span)
                | Cause::FunctionArgument(span)
                | Cause::Assignment(span) => bail!(TypeckError::TypesMissmatch {
                    related: vec![TypeckRelated::Here {
                        src: span.0.clone(),
                        span: span.1.clone().into()
                    }],
                    ...
                    bail!(TypeckError::TypesMissmatch {
                        related: vec![
                            TypeckRelated::ThisType {
                                src: a1.0.clone(),
                                span: a1.1.clone().into(),
                                t: p1.clone()
                            },
                            TypeckRelated::ThisType {
                                src: a2.0.clone(),
                                span: a2.1.clone().into(),
                                t: p2.clone()
                            }
                        ],
                        expected: p1,
